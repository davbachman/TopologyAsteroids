...
- Spherical layout update: widened disk separation by changing `SPHERE_DISK_LAYOUT` to radius `220` with centers `(250,320)` and `(890,320)` so the inter-disk gap is larger.
